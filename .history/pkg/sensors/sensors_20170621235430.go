...
func register(name string, factory SensorFactory) {
	...
	if registered {
		...
		panic("Sensor factory already registered. Ignoring.")
	}
...
func CreateSensor(conf map[string]string) (Sensor, error) {
	...
	if !found {
		...
		// return nil, fmt.Errorf("Invalid Datastore name. Must be one of: %s", strings.Join(availableSensors, ", "))
		panic("Invalid sensor mode.")
	}
