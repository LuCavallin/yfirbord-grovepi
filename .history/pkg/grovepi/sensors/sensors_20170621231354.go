...
func Register(type string, factory SensorFactory) {
	if factory == nil {
		log.Panicf("Sensor factory %s does not exist.", type)
	}
	_, registered := datastoreFactories[type]
	if registered {
		log.Errorf("Sensor factory %s already registered. Ignoring.", type)
	}
	datastoreFactories[type] = factory
}
